...
void main() {
  ...
  group('catalog entry', () {
    ...
    test('downloads the codec file as well as the backbone', () {
      // Both halves ship now. The entry used to fetch only the backbone and
      // ask the user to convert a codec file by hand, because the published
      // one carried no tokenizer and no voice encoder and named its head
      // `lm.c0_head.weight` where codec.cpp expects `lm.heads_0.weight`.
      // BricksDisplay's build fixes all three, so nothing is supplied by hand.
      final entry = catalog
          .byKind(ModelKind.tts)
          .firstWhere((m) => m.engine == TtsEngineKind.chatterboxGguf.name);
      ...
      expect(
        entry.files.map((f) => f.name),
        contains(ChatterboxGgufTtsService.codecFile),
        reason: 'the codec is downloaded, not supplied by the user',
      );
    });
  });
