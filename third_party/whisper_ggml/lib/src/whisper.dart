...
class Whisper {
  ...
  Future<WhisperTranscribeResponse> transcribe({
    ...
    try {
      final File inputFile = File(transcribeRequest.audio);
      // The app records 16 kHz mono PCM directly. Running that file through
      // FFmpeg again adds a second native runtime and a large transient memory
      // allocation immediately before whisper.cpp loads its model.
      final bool isWave = inputFile.path.toLowerCase().endsWith('.wav');
      final File? convertedFile = isWave
          ? inputFile
          : await WhisperAudioConvert(
