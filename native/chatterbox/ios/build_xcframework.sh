...
if [[ "$WANT_SIM" == "1" ]]; then
    ...
    #
    # One architecture, not the "arm64;x86_64" pair this asked for originally.
    # The isolated backbone is a nested ExternalProject and it does not honour
    # a multi-architecture CMAKE_OSX_ARCHITECTURES: it builds the first one and
    # the link of the second slice then fails with every llama_* symbol
    # undefined. The simulator only ever has to match the host Mac, so build
    # for that and let SIM_ARCH override it on an Intel machine.
    SIM_ARCH="${SIM_ARCH:-$(uname -m)}"
    METAL=OFF build_slice simulator iphonesimulator "$SIM_ARCH"
