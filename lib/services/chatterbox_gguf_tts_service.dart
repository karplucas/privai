...
class ChatterboxGgufTtsService implements TtsEngine {
  ...
  final AudioClipPlayer _player = AudioClipPlayer('ChatterboxGgufTtsService');

  /// Completed by [stop] so a clip cut short stops being awaited at once;
  /// `AudioPlayer.stop()` emits no completion event.
  Completer<void> _stopSignal = Completer<void>();

  /// Playback generation, so a stop abandons whatever is still queued.
  ...
  Future<void> speak(
    ...
    await _play(file, audio, _playbackEpoch);
  }

  /// Plays one rendered clip, awaited rather than left running: the caller
  /// unloads the language model to make room for this engine and reloads it in
  /// a `finally`, so returning early would cut the audio off as it started.
  Future<void> _play(File file, ChatterboxAudio audio, int epoch) async {
    ...
    try {
      await _player.play(
        file,
        expected: Duration(milliseconds: (audio.seconds * 1000).round()),
  ...
  Future<File> _writeWav(ChatterboxAudio audio) async {
    ...
    await file.writeAsBytes(
      encodeWav(
        _removeVocoderTone(audio.samples),
